...
function InternshipLetter({ showToast }) {
  ...
  const handleGenerate = async (event) => {
    ...
    try {
      ...
      const trimmedInternName = formData.internName.trim();
      const trimmedRole = formData.internRole.trim();
      ...
      payload.append('name', trimmedInternName);
      ...
      const placeholderValues = {
        ...
        intern_name: trimmedInternName,
        internship_duration: formData.internshipDuration.trim(),
        start_date: formData.startDate,
        end_date: formData.endDate,
        intern_role: trimmedRole,
        work_area_1: formData.workArea1.trim(),
        work_area_2: formData.workArea2.trim(),
      ...
      if (!response.ok) {
        let errorMessage = 'Unable to generate internship certificate.';
        try {
        ...
        console.error('Internship certificate generation failed:', {
      ...
      setDownloadUrl(url);
      triggerDownload(url);
      notify('Internship certificate generated successfully! Download started.', 'success');
    } catch (error) {
      console.error('Error generating internship certificate:', error);
      const errorMessage = error.message || 'Failed to generate internship certificate. Please try again.';
      ...
      <div className="documents-card">
        <div className="documents-header">
          ...
          <AutoSizeText as="p" className="documents-subtitle" min={12} max={18}>
            Upload a DOCX template file containing placeholders like {`{date}`}, {`{ref_no}`}, {`{intern_name}`},
            {`{internship_duration}`}, {`{start_date}`}, {`{end_date}`}, {`{intern_role}`}, {`{work_area_1}`},
            {`{work_area_2}`}, {`{signatory_name}`}, {`{signatory_designation}`}, or {`{ref.no}`}. Fill out every field to generate a
            personalized internship certificate PDF.
          </AutoSizeText>
        </div>
        ...
        <form className="documents-form" onSubmit={handleGenerate}>
          ...
          <div className="documents-grid">
            <div className="documents-field">
              <AutoSizeText as="label" htmlFor="internName" min={12} max={16}>
                Intern Name <span style={{ color: '#e74c3c' }}>*</span>
              </AutoSizeText>
              <input
                id="internName"
                name="internName"
            ...
            <div className="documents-field">
              <AutoSizeText as="label" htmlFor="internshipDuration" min={12} max={16}>
                Internship Duration <span style={{ color: '#e74c3c' }}>*</span>
              </AutoSizeText>
              <input
          ...
          <div className="documents-grid">
            <div className="documents-field">
              <AutoSizeText as="label" htmlFor="internRole" min={12} max={16}>
                Intern Role <span style={{ color: '#e74c3c' }}>*</span>
              </AutoSizeText>
              <input
                id="internRole"
                name="internRole"
            ...
            <div className="documents-field">
              <AutoSizeText as="label" htmlFor="workArea1" min={12} max={16}>
                Work Area 1 <span style={{ color: '#e74c3c' }}>*</span>
              </AutoSizeText>
              <input
                id="workArea1"
                name="workArea1"
                type="text"
                value={formData.workArea1}
                onChange={handleFieldChange}
          ...
          <div className="documents-grid">
            ...
            <div className="documents-field">
              <AutoSizeText as="label" htmlFor="workArea2" min={12} max={16}>
                Work Area 2 <span style={{ color: '#e74c3c' }}>*</span>
              </AutoSizeText>
              <input
                id="workArea2"
                name="workArea2"
                type="text"
                value={formData.workArea2}
                onChange={handleFieldChange}
                placeholder="e.g. Client presentations"
                required
              />
            </div>
            <div className="documents-field">
